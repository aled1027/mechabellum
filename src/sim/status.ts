import type { SimEvent, StatusEffectType, UnitInstance } from "./types.js";

export const addStatusEffect = (
  target: UnitInstance,
  effect: { type: StatusEffectType; durationTicks: number; magnitude: number },
...
export const tickStatusEffects = (unit: UnitInstance, events: SimEvent[], tick: number): void => {
...
export const hasStatus = (unit: UnitInstance, type: StatusEffectType): boolean =>
  unit.statusEffects.some((status) => status.type === type);

export const getMoveSpeedMultiplier = (unit: UnitInstance): number => {
...
export const getArmorMultiplier = (unit: UnitInstance): number => {
