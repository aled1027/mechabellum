...
export const generateShopOffers = (data, rng, config = defaultShopConfig, lockedUnitOffer) => {
    ...
    const unitOffers = pickWeightedUnits(remainingUnits, config.unitOfferCount - (lockedUnitOffer ? 1 : 0), rng, config.unitTierWeights).map((unit, index) => ({
    ...
    const cardOffers = pickWeightedCards(data.cards, config.cardOfferCount, rng, config.cardRarityWeights).map((card, index) => ({
...
export const purchaseTechOffer = (data, state, economy, roster, offerId, unitId, config = defaultShopConfig) => {
...
export const purchaseCardOffer = (data, state, economy, cardState, offerId, config = defaultShopConfig) => {
