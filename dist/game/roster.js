...
export const defaultCardPurchaseConfig = {
    maxCardsPerRound: 2
};
export const createRosterState = () => ({
    units: [],
...
const generateUnitInstanceId = (definitionId, index) => {
    return `${definitionId}-${index}`;
};
const canAddUnit = (roster, unit, limits) => {
...
export const purchaseUnit = (economy, roster, unit, supplyConfig = defaultSupplyConfig, limits = defaultLimits) => {
...
export const purchaseUpgrade = (economy, roster, unitId, upgradeConfig = defaultUpgradeConfig) => {
...
export const purchaseTech = (economy, roster, unitId, tech, allTechs, fallbackCost) => {
    const unitIndex = roster.units.findIndex((unit) => unit.id === unitId);
    if (unitIndex < 0) {
        return { success: false, error: "Unit not found" };
    }
...
export const purchaseCard = (economy, cardState, card, fallbackCost, config = defaultCardPurchaseConfig) => {
