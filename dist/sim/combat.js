...
import { addStatusEffect, getArmorMultiplier, getMoveSpeedMultiplier, hasStatus, tickStatusEffects } from "./status.js";
...
export class CombatSimulator {
    ...
    init(input) {
        ...
        for (const placement of input.placements) {
            ...
            const offsets = buildSquadOffsets(definition.stats.squadSize, definition.stats.collisionSize, facing);
            offsets.forEach((offset, index) => {
                const id = `${placement.side}-${definition.id}-${placement.position.x}-${placement.position.y}-${index}`;
                units.push(this.buildUnit(definition, placement.position, facing, id, offset, placement.side));
            });
        }
        this.state = { tick: 0, units, projectiles: [], wreckage: [] };
    }
    buildUnit(definition, position, orientation, id, offset, side) {
        return {
            id,
    ...
    step(tick) {
        ...
        this.state.projectiles = updateProjectiles(this.state.projectiles, this.state.units, this.events, tick);
    ...
    updateMovement() {
        for (const unit of this.sortedUnits()) {
            ...
            if (goal) {
                const path = findPath(this.snapToGrid(unit.position), goal, (pos) => this.isBlocked(pos, unit));
                unit.path = path;
            }
    ...
    updateTargeting() {
        ...
        for (const unit of this.sortedUnits()) {
            ...
            const candidates = this.state.units.filter((candidate) => candidate.side !== unit.side && candidate.hp > 0);
    ...
    updateFiring(tick) {
        ...
        for (const unit of this.sortedUnits()) {
            ...
            if (!hit) {
                ...
                applyDamage(target, unit.stats.damage, unit.stats.attackType, this.events, tick, getArmorMultiplier(target));
