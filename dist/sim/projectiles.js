...
export const updateProjectiles = (projectiles, units, events, tick) => {
    ...
    for (const projectile of projectiles) {
        ...
        if (hitTarget || projectile.remainingTicks <= 0) {
            const impactedUnits = projectile.aoeRadius > 0
                ? units.filter((unit) => distance(unit.position, projectile.position) <= projectile.aoeRadius)
                : target
                    ? [target]
                    : [];
            for (const impacted of impactedUnits) {
                applyDamage(impacted, projectile.damage, projectile.attackType, events, tick, getArmorMultiplier(impacted));
